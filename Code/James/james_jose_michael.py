...
class Character():
    def __init__(self, power= 10, name= 'Jeff', health= 10, x = 1, y = 3, experience= 0):
        self.x = x
        self.y = y
        self.power = power
        self.name = name
...
player.y=4
enemy = Enemy()
enemy.coins = random.randint(0, 100)
player.experience = 0
...
while True:
    ...
    if action == 'attack':
        ...
        enemies -= 1
        player.experience += enemy.coins # add experience
        print('You have gained ' + str(enemy.coins) + ' experience')
        action = 'None' # reset action
        if enemies == 0:
            print('Congrats you won!')
            print('You have ' + str(player.experience) + ' experience')
            break
