...
class ContactList:
    ...
    def load(self):
        ...
        with open(r'C:\Users\rm210\Desktop\codepdx\extra\contacts.json') as self.contacts: # r had to be added in front of string to eliminate ('SyntaxError: (unicode error) 'unicodeescape' codec can't decode bytes in position 2-3: truncated')
           
        # 2) get the text from the file

            self.contacts = self.contacts.read()
            #print(type(self.contacts))
            #print(self.contacts)
        # 3) convert the text into a python dictionary (json.loads)
        
            self.contacts = json.loads(self.contacts)
            #print(type(self.contacts))
            #print(self.contacts)
