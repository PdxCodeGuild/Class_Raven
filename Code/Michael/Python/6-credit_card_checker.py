...
while invalid_cc_number:
    cc_numbers = input("Please enter a cc number seperated by spaces or commas: ").replace(" ", "").replace(",","") # Removes spaces and commas.
...
for number in cc_numbers: 
    if number > 9: # Subtract nine from numbers over nine.
        cc_number_nined.append(number-9) # Place into new list because I had trouble figuring out how to do it in same list.
    else:
        cc_number_nined.append(number)
    
summed_cc = str(sum(cc_number_nined)) # Sum the list and convert sum to string to use as list.

print("Check Digits: ", summed_cc)
print("Comparative Digit: ", summed_cc[len(summed_cc)-1])
print("Check Digit: ", check_digit)

if check_digit == summed_cc[len(summed_cc)-1]: # If check digit and comparitive digit match then valid.
